        ...
        <div class="flex items-center">
            ...
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    {{ __('Create New Transportation Service') }}
                </h2>
                <p class="text-gray-600 mt-1">Fill in the details to create a new transportation service</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('admin.transportation.services.store') }}" method="POST" class="p-6 space-y-6" x-data="serviceForm()">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="service_type" 
                            ...
                            <option value="shared_ride" {{ old('service_type') === 'shared_ride' ? 'selected' : '' }}>
                                Shared Ride
                            </option>
                            <option value="solo_ride" {{ old('service_type') === 'solo_ride' ? 'selected' : '' }}>
                                Solo Ride
                            </option>
                            <option value="airport_transfer" {{ old('service_type') === 'airport_transfer' ? 'selected' : '' }}>
                                Airport Transfer
                            </option>
                            <option value="car_hire" {{ old('service_type') === 'car_hire' ? 'selected' : '' }}>
                                Car Hire
                            </option>
                            <option value="parcel_delivery" {{ old('service_type') === 'parcel_delivery' ? 'selected' : '' }}>
                                Parcel Delivery
                            </option>
                        </select>
                        @error('service_type')
                    ...
                    <div>
                        ...
                        <select id="pricing_model" 
                            ...
                            <option value="city_based" {{ old('pricing_model') === 'city_based' ? 'selected' : '' }}>
                                City Based
                            </option>
                            <option value="vehicle_city_based" {{ old('pricing_model') === 'vehicle_city_based' ? 'selected' : '' }}>
                                Vehicle & City Based
                            </option>
                            <option value="vehicle_transfer_based" {{ old('pricing_model') === 'vehicle_transfer_based' ? 'selected' : '' }}>
                                Vehicle & Transfer Based
                            </option>
                            <option value="time_based" {{ old('pricing_model') === 'time_based' ? 'selected' : '' }}>
                                Time Based
                            </option>
                            <option value="weight_location_based" {{ old('pricing_model') === 'weight_location_based' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea id="description" 
                                  ...
                                  placeholder="Describe this transportation service...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <script>
        function serviceForm() {
            return {
                ...
                pricingModel: '{{ old('pricing_model') }}',
                ...
                updatePricingModel() {
                    ...
                    // Auto-select appropriate pricing model based on service type
                    switch (this.serviceType) {
                        case 'shared_ride':
                            this.pricingModel = 'city_based';
                            break;
                        case 'solo_ride':
                            this.pricingModel = 'vehicle_city_based';
                            break;
                        case 'airport_transfer':
                            this.pricingModel = 'vehicle_transfer_based';
                            break;
                        case 'car_hire':
                            this.pricingModel = 'time_based';
                            break;
                        case 'parcel_delivery':
                            this.pricingModel = 'weight_location_based';
                            break;
                    }
                }
            }
